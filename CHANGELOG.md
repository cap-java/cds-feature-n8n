...
- Supports CDS entity events (`CREATE`, `DELETE`) and custom actions/functions as trigger sources
...
- Optional `inputs` payload selection on `@n8n.process.start`; when omitted, all scalar entity fields are sent automatically
